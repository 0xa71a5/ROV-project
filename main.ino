...
void MotorAttach()
{
  motor1.attach(2,motor1_min_pwm,motor1_max_pwm);
  motor2.attach(3,motor2_min_pwm,motor2_max_pwm);
  motor3.attach(4,motor3_min_pwm,motor3_max_pwm);
...
void setHorizonPressure(float input)
{
  ...
  horizonPressure=input;
    Serial.print("Horizon pressure set to ");
    Serial.println(horizonPressure);
}
    ...
    if(commandHead.length()==1)//command head is just one character,this shorten the time of command resolving
    {
      ...
      switch(commandType)
      {
          ...
          case 'B'://set max pwm of motor1
            if(commandHead.length()==3)
            {
              uint8_t motorIndex=commandHead[1]-'0';//'0' or '1' or '2' or '3'
